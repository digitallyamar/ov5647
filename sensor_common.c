	...
	struct sensor_signal_properties *signal)
{
	...
	printk("sensor_common_parse_signal_props++");
	...
	printk("sensor_common_parse_signal_props: signal->readout_orientation = %d\n", signal->readout_orientation);
	...
	dev_info(dev, "%s: signal->num_lanes = %d\n", __func__, signal->num_lanes);
	printk("%s: signal->num_lanes = %d\n", __func__, signal->num_lanes);
	...
	dev_info(dev, "%s: signal->mclk_freq = %d\n", __func__, signal->mclk_freq);
	...
	dev_info(dev, "%s: signal->pixel_clock.val = %llu\n", __func__, signal->pixel_clock.val);
	...
	dev_info(dev, "%s: signal->serdes_pixel_clock.val = %llu\n", __func__, signal->serdes_pixel_clock.val);
	...
	dev_info(dev, "%s: signal->cil_settletime = %d\n", __func__, signal->cil_settletime);
	...
	dev_info(dev, "%s: signal->discontinuous_clk = %d\n", __func__, signal->discontinuous_clk);
	...
	dev_info(dev, "%s: signal->dpcm_enable = %d\n", __func__, signal->dpcm_enable);
	...
	dev_info(dev, "%s: signal->deskew_initial_enable = %d\n", __func__, signal->deskew_initial_enable);
	...
	dev_info(dev, "%s: signal->deskew_periodic_enable = %d\n", __func__, signal->deskew_periodic_enable);
	...
	dev_info(dev, "%s: signal->tegra_sinterface = %d\n", __func__, signal->tegra_sinterface);
	...
	dev_info(dev, "%s: signal->phy_mode = %d\n", __func__, signal->phy_mode);
...
int sensor_common_parse_num_modes(const struct device *dev)
{
	...
	printk("Amar: sensor_common_parse_num_modes++\n");
	...
	printk("Amar: sensor_common_parse_num_modes--, num_modes = %d\n", num_modes);
	...
	struct sensor_cfg *cfg)
{
	...
	if (tmp != NULL) {
		dev_info(dev, "%s: [Amar] Calling sensor_common_init_i2c_device_config", __func__);
		err = sensor_common_init_i2c_device_config(dev, np, cfg);
		if (err)
			goto exit;
		dev_info(dev, "%s: [Amar] Success - sensor_common_init_i2c_device_config", __func__);
	}
...
exit:
	dev_err(dev, "%s: [Amar] Return value %d from sensor_common_init_i2c_device_config", __func__, err);
	of_node_put(parent);
	return err;
}
	...
	struct sensor_properties *sensor)
{
	...
	printk("Amar: [sensor_common.c] sensor_common_init_sensor_properties++");
	...
	for (i = 0; i < num_modes; i++) {
		...
		printk("Amar: [sensor_common.c] Calling => sensor_common_parse_signal_props");
		...
		printk("Amar: [sensor_common.c] Calling => sensor_common_parse_image_props");
		...
		printk("Amar: [sensor_common.c] Calling => sensor_common_parse_dv_timings");
		...
		printk("Amar: [sensor_common.c] Calling => sensor_common_parse_control_props");
