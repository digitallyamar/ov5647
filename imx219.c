...
static int imx219_board_setup(struct imx219 *priv)
{
	...
	dev_info(dev, "%s, IMX219_MODEL_ID_ADDR_MSB Register value = %d\n", __func__, reg_val[0]);
	...
	dev_info(dev, "%s, IMX219_MODEL_ID_ADDR_LSB Register value = %d\n", __func__, reg_val[1]);


	// OV5647 Start

					err = imx219_read_reg(s_data, OV5647_SC_CMMN_CHIP_ID_MSB, &reg_val[1]);
					if (err) {
						dev_err(dev, "%s: error during i2c read probe (%d)\n",
							__func__, err);
						goto err_reg_probe;
					...
					err = imx219_read_reg(s_data, OV5647_SC_CMMN_CHIP_ID_LSB, &reg_val[1]);
					if (err) {
						dev_err(dev, "%s: error during i2c read probe (%d)\n",
							__func__, err);
						goto err_reg_probe;
	...
	const struct i2c_device_id *id)
{
	...
	dev_info(dev, "Amar: imx219_probe++");
    dev_info(dev, "Amar:(imx219_probe) i2c_client->name = %s\n", client->name);
    dev_info(dev, "Amar:(imx219_probe) i2c_client->dev = %s\n", dev->init_name);
