...
int tegracam_device_register(struct tegracam_device *tc_dev)
{
	...
	dev_info(dev, "Amar:add tegracam sensor driver:%s_v%d.%d.%d\n",
	...
	printk("Amar::tg_core: Calling devm_regmap_init_i2c\n");
	...
	printk("Amar::tg_core: devm_regmap_init_i2c done\n");
	...
	printk("Amar::tg_core: Calling sensor_ops->parse_dt\n");

	s_data->pdata = tc_dev->sensor_ops->parse_dt(tc_dev);

	printk("Amar::tg_core: Checking sensor_ops->parse_dt return\n");
	...
	printk("Amar::tg_core: Done checking sensor_ops->parse_dt return\n");

	tc_dev->s_data = s_data;

	printk("Amar::tg_core: Calling sensor_ops->power_get return\n");
	err = tc_dev->sensor_ops->power_get(tc_dev);
	printk("Amar::tg_core: Done calling sensor_ops->power_get return\n");
	if (err) {
		dev_err(dev, "unable to power get\n");
		return -EFAULT;
	}

	printk("Amar::tg_core: Calling camera_common_initialize\n");
	err = camera_common_initialize(s_data, tc_dev->name);
	printk("Amar::tg_core: Done calling camera_common_initialize\n");
	if (err) {
...
void tegracam_device_unregister(struct tegracam_device *tc_dev)
{
	...
	dev_info(dev, "%s - Calling power_put() \n", __func__);
	tc_dev->sensor_ops->power_put(tc_dev);
	dev_info(dev, "%s - Calling camera_common_cleanup() \n", __func__);
	camera_common_cleanup(s_data);

	dev_info(dev, "%s -  Remove tc_dev from list of registered devices  \n", __func__);
